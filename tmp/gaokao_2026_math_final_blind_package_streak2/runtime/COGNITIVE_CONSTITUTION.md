...
If the honest quantity is already nameable,
the next local spend should tend to bind it to one same-carrier object-side operation.

Until that happens, the quantity is still only provisionally recognized.
...
The important point is not merely that all three exist.

They must couple.

If monitoring, doubt, non-closure, conflict, or counter-question pressure is real,
it should not remain only as sayable content.

It should purchase a control change:

- one local object-side move becomes easier
- at least one decorative continuation becomes harder
- and the current commitment object keeps authority until one same-carrier change happens or the line is killed

If these signals can be named but do not reallocate control locally,
the system is still knowing without doing.
...
### 19C2. Working Memory Is A Soft Binding Field

Working memory here is not a queue of steps.

It is a soft binding field:

- foreground keeps the current commitment object lightly resident
- foreground also keeps one writable candidate facet, witness, or output surface lightly resident
- non-winning lines cool unless they sharpen legality, rivalry, or readout on that same boundary

The target is not to remember everything equally.

The target is to keep only enough bound structure alive
for the next honest move and its nearest revocation handle.
